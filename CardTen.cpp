...
int CardTen::NumberOf10Cards = 0;
int CardTen::SavedCards = 0;
...
bool CardTen::IsValidCard() const
{
	if (Price > 0 && Fees > 0) return true;
	return false;
}

void CardTen::Save(ofstream& save, ObjectType T)
{
	if (T != Cards) return;
	...
	if (++SavedCards == 1)
		save << " " << Price << " " << Fees;
	save << "\n";
	if (SavedCards == NumberOf10Cards) SavedCards = 0;
}

void CardTen::Read(ifstream& load)
{
	int cellpos,p,f;
	load >> cellpos;
...
void CardTen::RemoveOwnership()
{
	Owner = NULL;
}

void CardTen::EditParameters(Grid* pGrid)
{
