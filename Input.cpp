...
int Input::GetInteger(Output *pO) const 
{
	...
	for (int i = 0; i < toint.length(); i++) if (toint[i] < '0' || toint[i]>'9') return -1;
	int toreturn = stoi(toint);



	// Note: stoi(s) converts string s into its equivalent integer (for example, "55" is converted to 55)

	return toreturn; // this line should be changed with your implementation
}
